                    ...
                    <c:if test="${list != null}">
                        <table class="table mt-4 text-white">
                            ...
                            <c:forEach items="${list}" var="book" >
                                <tr>
                                    <td>${book.id}</td>
                                    <td><a href="${url}/view-book-detail?bid=${book.id}">${book.name}</a></td>
                                    <td>${book.price}</td>
                                    <td>${book.catename}</td>
                                    <td>
                                        <c:choose>
                                    ...
                                    <td>
                                        <c:choose>
                                        <c:when test="${book.status == 1}">
                                            <a class="btn btn-danger" href="${url}/change-book-status?status=0&bookid=${book.id}">
                                                Hết hàng</a> 
                                        </c:when>
                                         <c:when test="${book.status == 0}">
                                            <a class="btn btn-success" href="${url}/change-book-status?status=1&bookid=${book.id}">
                                                Còn hàng</a> 
                                        </c:when>
                                        </c:choose>
                                    </td>
